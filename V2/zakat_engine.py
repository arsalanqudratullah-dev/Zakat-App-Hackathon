...
def classify_debts(debts_df):
    ...
    for _, row in debts_df.iterrows():
        ...
        total += balance
        interest_bearing = str(row.get("interest_bearing", "")).lower() == "yes"
        exp = "Hanbali: full outstanding balance subtracted, no 12-month cap."
        if interest_bearing:
            exp += (" This debt is interest-bearing (riba) — the interest itself is impermissible, "
                     "but the outstanding balance is still a real liability and is fully subtracted here.")
        results.append({"debt_id": row["debt_id"], "creditor": row["creditor"], "keyword": row["keyword"],
                         "outstanding_balance_cad": balance, "interest_bearing": interest_bearing,
                         "explanation": exp})
